...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def __init__(self, application, request, transforms=None):
        ...
        logging.debug("BaseHandler__init__  path %s" % (self.request.path))
        self.session = {}
        self._messages = []
        self.msg_errors = []
        self.msg_alerts = []
        ...
        if 'HTTP_X_FORWARDED_FOR' in request.environ:
            ...
            pass
        """
        logging.debug("end:  %s" % (self.current_user))
        self.__before__()
    ...
    def get_current_site(self):
        ...
        if 'apikey' in self.request.arguments:
            site = model.site.Site.by_apikey(self.request.arguments['apikey'])
        else:
            user = self.get_current_user()
            if user:
    ...
    def set_current_user(self,sauser,remember_me = False,is_authenticated = False, islogon=True):
        ...
        # don't add this is_authenticated to redis!!! Or memcached
        ...
        self.set_cookie("userkey",sauser.user_uniqueid)
